CREATE OR REPLACE TABLE `tech-chalenge-covid`.pnad_covid_processed.pessoas_052020_processed AS
SELECT
    ...
    CAST(raw.A002 AS INT64) AS Idade, -- Manter NULL se houver
    ...
    CAST(raw.C01012 AS NUMERIC) AS RendimentoHabitual_Valor, -- Manter NULL se for o caso
...
FROM
    `tech-chalenge-covid`.pnad_covid_raw.pessoas_052020_raw AS raw
-- JOINs com as tabelas de domínio (mesmos JOINs de antes)
